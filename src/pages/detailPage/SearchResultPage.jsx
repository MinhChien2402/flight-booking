...
const SearchResultsPage = () => {
  ...
  // Đồng bộ filteredTickets với outboundTickets và returnTickets từ Redux
  useEffect(() => {
    console.log("useEffect triggered with outboundTickets:", outboundTickets);
    console.log("useEffect triggered with returnTickets:", returnTickets);
    const newFilteredTickets = [
      ...(Array.isArray(outboundTickets) ? outboundTickets : []),
      ...(Array.isArray(returnTickets) ? returnTickets : []),
    ];
    console.log("Updated filteredTickets:", newFilteredTickets);
    setFilteredTickets(newFilteredTickets);
  }, [outboundTickets, returnTickets]);
  ...
  const handleBookNow = (flight) => {
    ...
    const bookingFlight = {
      ...
      departTime: flight.departTime || "N/A",
      departDate: flight.departDate || "N/A",
      from: flight.from || "N/A",
      arriveTime: flight.arriveTime || "N/A",
      arriveDate: flight.arriveDate || "N/A",
      to: flight.to || "N/A",
      duration: flight.duration || "N/A",
      stops: flight.stops || "N/A",
      price: flight.price || "$0.00",
      refundable: flight.refundable || "N/A",
      availableSeats: flight.availableSeats || "N/A",
    };
    navigate("/review-booking", {
      state: { flight: bookingFlight, searchParams },
    });
  };
  ...
  const handleSelectTicket = (ticket, isOutbound) => {
    if (isOutbound) {
      ...
      setFilteredTickets(filteredTickets.filter((t) => t.id !== ticket.id));
    } else {
  ...
  const formatFlightData = (ticket) => {
    console.log("Formatting ticket:", ticket);
      ...
      <div className="max-w-6xl mx-auto p-4">
        ...
        <div className="mb-6">
          ...
          {selectedOutboundTicket && (
            <div className="mb-4">
              <h4 className="text-lg font-medium text-gray-700">Chuyến đi</h4>
              {(() => {
                    ...
                    <div className="flex items-center justify-between w-full">
                      ...
                      <div className="flex flex-col items-end w-1/5 text-right text-sm space-y-1">
                        ...
                        <div className="flex justify-end space-x-2 mt-2">
                          ...
                          >
                            {openDetails === flight.id ? "Hidden" : "Details"}
                          </button>
                        </div>
                      </div>
                    </div>
          ...
          {selectedReturnTicket && (
            <div className="mb-4">
              <h4 className="text-lg font-medium text-gray-700">Round trip</h4>
              {(() => {
        ...
        {Array.isArray(filteredTickets) && filteredTickets.length > 0 && (
          <>
            ...
            {filteredTickets.map((ticket) => {
              const isSelected =
                selectedOutboundTicket?.id === ticket.id ||
                selectedReturnTicket?.id === ticket.id;
              if (isSelected) return null;
              const flight = formatFlightData(ticket);
              const isOutbound = outboundTickets.some(
                (t) => t.id === ticket.id
              );
              return (
                <div
                      ...
                      <div className="flex justify-end space-x-2 mt-2">
                        <Button
                          ...
                          onClick={() => {
                            ...
                            handleSelectTicket(ticket, isOutbound);
                          }}
                        >
                          Book Now
                        </Button>
